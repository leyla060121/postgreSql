...
select * from calisanlar; --tablosu için birleşme yeri isyeri sutunu
select * from markalar; -- tablosu için birleşme yeri marka_isim sutunu
...
select isim, maas, isyeri from calisanlar       --soruda isyeri istemiyor çıktıda görmek için yazdık
where isyeri in (select marka_isim from markalar where calisan_sayisi>15000) 		--bağlantı noktası
--in kullanacaksanız parantez içine aldığınız sorgu birden fazla değer döndürmeli
